...
public class CategoriaRepository(AppDbContext context) : ICategoriaRepository
{
    ...
    /// <summary>
    /// Obtém todas as categorias que possuem produtos associados de forma assíncrona.
    /// </summary>
    /// <returns>
    /// Uma tarefa que representa a operação assíncrona. O resultado da tarefa
    /// contém uma lista de entidades <see cref="Categoria"/> que possuem produtos associados.
    /// </returns>
    public async Task<IEnumerable<Categoria>> ObterCategoriasComProdutosAsync()
    {
        return await _context.Categorias
            .Where(c => c.Produtos != null && c.Produtos.Any())
            .ToListAsync();
    }
    ...
    /// <exception cref="DbException">Ocorre se houver um erro ao acessar o banco de dados.</exception>
    public async Task<bool> CategoriaPossuiProdutosAssociadosAsync(string categoriaId)
    {
    ...
    /// <exception cref="DbException">Ocorre se houver um erro ao acessar o banco de dados.</exception>
    public async Task<bool> ExisteCategoriaComNomeAsync(string nome, string id = "")
    {
